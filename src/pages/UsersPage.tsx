...
export const UsersPage: React.FC = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const usrs = qs.docs.map(d => ({ id: d.id, ...d.data() } as UserProfile));
      setUsers(usrs);
    } catch (error) {
  ...
  const handleDirectRoleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateDoc(doc(db, 'users', directRoleUserId), { role: directRoleValue });
      setUsers(users.map(u => u.id === directRoleUserId ? { ...u, role: directRoleValue as any } : u));
      setDirectRoleUserId('');
      setDirectRoleValue('');
      alert('Права успешно выданы');
    } catch (error) {
      handleFirestoreError(error, OperationType.UPDATE, `users/${directRoleUserId}`);
    }
  };
